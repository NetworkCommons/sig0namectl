  ...
  globalThis.Go = class {
    constructor() {
      ...
      this.importObject = {
        ...
        gojs: {
          // Go's SP does not change as long as no Go code is running. Some
          // operations (e.g. calls, getters and setters) may synchronously
          // trigger a Go event handler. This makes Go code get executed in the
          // middle of the imported function. A goroutine can switch to a new
          // stack if the current stack is too small (see morestack function).
          // This changes the SP, thus we have to update the SP used by the
          // imported function.
          ...
          'runtime.wasmWrite': (sp) => {
            ...
            fs.writeSync(
                fd, new Uint8Array(this._inst.exports.mem.buffer, p, n));
          },
                ...
                setTimeout(
                    () => {
                      ...
                      while (this._scheduledTimeouts.has(id)) {
                        // for some reason Go failed to register the timeout
                        // event, log and try again (temporary workaround for
                        // https://github.com/golang/go/issues/28975)
                        console.warn(
          ...
          'syscall/js.valueIndex': (sp) => {
            sp >>>= 0;
            storeValue(
                sp + 24, Reflect.get(loadValue(sp + 8), getInt64(sp + 16)));
          },
    ...
    async run(instance) {
      ...
      this.mem = new DataView(this._inst.exports.mem.buffer);
      this._values = [
        // JS values that Go currently has references to, indexed by reference
        // id
        NaN,
        0,
        null,
      ...
      this._goRefCounts =
          new Array(this._values.length)
              .fill(Infinity);  // number of references that Go has to a JS
                                // value, indexed by reference id
      this._ids = new Map([
